...
def dish_predict(dish):
    ...
    predictions_dish = 24  # на следующие 120 часов
    ...
    future_dish = future_dish[['ds', 'yhat_upper', 'yhat']]
...
def predict_all(list_dishes):
    p = Pool(cpu_count())
    predictions = list(tqdm(p.imap(dish_predict, list_dishes), total=len(list_dishes)))
    p.close()
    p.join()
    print(predictions)
